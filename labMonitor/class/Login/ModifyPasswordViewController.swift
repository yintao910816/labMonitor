...
class ModifyPasswordViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        if let s = UserManager.shareIntance.currentUser?.phonenum, s.count > 0{
            cellphoneTF.text = s
        }else {
            NSObject.alert(message: "您还未绑定手机号，请绑定手机号后再修改", cancleTitle: "下次再说", okTitle: "现在就去", callBackCancle: { [unowned self] in
                self.navigationController?.popViewController(animated: true)
            }) { [unowned self] in
                let setPhoneVC = ModifyPhoneViewController()
                self.navigationController?.pushViewController(setPhoneVC, animated: true)
            }
        }
    }
    ...
    func initRegisterV(){
        ...
        cellphoneTF.placeholder = "请输入您的手机号"
        ...
        verifyBtn.addTarget(self, action: #selector(ModifyPasswordViewController.startCount), for: UIControlEvents.touchUpInside)
        ...
        seeBtn.addTarget(self, action: #selector(ModifyPasswordViewController.passwordStyle), for: UIControlEvents.touchUpInside)
        ...
        registerBtn.addTarget(self, action: #selector(ModifyPasswordViewController.register), for: UIControlEvents.touchUpInside)
    ...
    @objc func startCount(){
        ...
        HttpRequestManager.shareIntance.HC_validateCode(userphone: cellphoneTF.text!) { [weak self](bool, msg) in
            ...
            if bool {
                ...
                self?.count = 0
                self?.timer = Timer.scheduledTimer(timeInterval: 1, target: self!, selector: #selector(ModifyPasswordViewController.showSecond), userInfo: nil, repeats: true)
            }else{
    ...
    @objc func register(){
        ...
        HttpRequestManager.shareIntance.HC_modifyPassword(UserId: id, newpassword: passwordTF.text!, messagecode: verifyTF.text!) { [weak self](bool, msg) in
            if bool{
